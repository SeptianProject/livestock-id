...
try {
    ...
    $userStmt = $pdo->query(
     ...
     WHERE u.role <> 'admin' AND p.id_user IS NULL
    ...
    if ($errors === []) {
        try {
            $insertStmt = $pdo->prepare(
                'INSERT INTO tb_petugas (nama_petugas, id_jabatan, id_user, no_telp)
         VALUES (:nama_petugas, :id_jabatan, :id_user, :no_telp)'
            );

            $insertStmt->execute([
                'nama_petugas' => $formData['nama_petugas'],
                'id_jabatan' => (int) $formData['id_jabatan'],
        ...
        <aside class="sidebar" id="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <ul style="list-style: none; padding: 0; margin: 0">
                    ...
                    <li class="nav-item"><a href="index.php" class="nav-link-item active"><i class="bi bi-people"></i><span>Petugas</span></a></li>
                </ul>
                <p class="nav-section-label">Pengaturan</p>
                <ul style="list-style: none; padding: 0; margin: 0">
                    <li class="nav-item"><a href="../jabatan/index.php" class="nav-link-item"><i class="bi bi-briefcase"></i><span>Jabatan</span></a></li>
                </ul>
                <p class="nav-section-label">Pencatatan</p>
                <ul style="list-style: none; padding: 0; margin: 0">
                    <li class="nav-item"><a href="#" class="nav-link-item"><i class="bi bi-heart-pulse"></i><span>Rekam Kesehatan</span></a></li>
                    <li class="nav-item"><a href="#" class="nav-link-item"><i class="bi bi-journal-text"></i><span>Catatan Produksi</span></a></li>
                </ul>
            </nav>
        ...
        <div class="main-area">
            <header class="topbar">
                ...
                <div style="display: flex; align-items: center; gap: 8px; flex: 1">
                    <a href="index.php" style="color: #7c8493; font-size: 13px; display: flex; align-items: center; gap: 4px;"><i class="bi bi-chevron-left"></i> Petugas</a>
                    <i class="bi bi-chevron-right" style="font-size: 11px; color: #b0b8c4"></i>
                    <span class="topbar-title" style="font-size: 15px">Tambah Petugas</span>
                </div>
                <div class="topbar-actions">
                    <div class="topbar-notif"><i class="bi bi-bell"></i><span class="notif-badge"></span></div>
            ...
            <main class="page-content">
                <div class="page-header">
                    ...
                    <p>Daftarkan petugas baru dan, jika sudah ada, hubungkan dengan akun user yang sesuai.</p>
                </div>
                    ...
                    <div class="form-card">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                            <div>
                                ...
                                <input type="text" id="nama_petugas" name="nama_petugas" class="form-control-custom" placeholder="Masukkan nama petugas" maxlength="100" required value="<?php echo e($formData['nama_petugas']); ?>" />
                            </div>
                            <div>
                                <label class="form-label" for="id_jabatan">Jabatan <span style="color: #e05252">*</span></label>
                                <select id="id_jabatan" name="id_jabatan" class="form-control-custom" required>
                                    <option value="" disabled <?php echo e($formData['id_jabatan'] === '' ? 'selected' : ''); ?>>Pilih jabatan</option>
                                    <?php foreach ($jabatans as $jabatan): ?>
                                        <option value="<?php echo e((string) $jabatan['id_jabatan']); ?>" <?php echo e($formData['id_jabatan'] === (string) $jabatan['id_jabatan'] ? 'selected' : ''); ?>><?php echo e($jabatan['nama_jabatan']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="id_user" name="id_user" class="form-control-custom">
                                    <option value="" <?php echo e($formData['id_user'] === '' ? 'selected' : ''); ?>>Tanpa akun</option>
                                    <?php foreach ($users as $user): ?>
                                        <option value="<?php echo e((string) $user['id_user']); ?>" <?php echo e($formData['id_user'] === (string) $user['id_user'] ? 'selected' : ''); ?>><?php echo e($user['username'] . ' (' . $user['role'] . ')'); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="tel" id="no_telp" name="no_telp" class="form-control-custom" placeholder="Contoh: +62 812-3456-7890" maxlength="20" required value="<?php echo e($formData['no_telp']); ?>" />
                            </div>
                        </div>

                        <div class="form-actions">
                            <button type="submit" class="btn-primary-custom"><i class="bi bi-check-lg"></i> Simpan Petugas</button>
