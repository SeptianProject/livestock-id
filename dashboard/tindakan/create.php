...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $formData['nama_tindakan'] = trim((string) ($_POST['nama_tindakan'] ?? ''));
    ...
    if ($errors === []) {
        try {
            $checkStmt = $pdo->prepare('SELECT COUNT(*) FROM tb_tindakan WHERE LOWER(nama_tindakan) = LOWER(:nama_tindakan)');
            $checkStmt->execute(['nama_tindakan' => $formData['nama_tindakan']]);
    ...
    if ($errors === []) {
        try {
            $insertStmt = $pdo->prepare('INSERT INTO tb_tindakan (nama_tindakan) VALUES (:nama_tindakan)');
            $insertStmt->execute(['nama_tindakan' => $formData['nama_tindakan']]);

            if ($action === 'save_new') {
                $successMessage = 'Data tindakan berhasil disimpan. Silakan tambah data baru.';
                $formData = ['nama_tindakan' => ''];
            } else {
                header('Location: index.php?success=Data tindakan berhasil ditambahkan');
                exit;
            }
        } catch (Throwable $exception) {
            error_log('Gagal menyimpan tindakan: ' . $exception->getMessage());
...
<body>
    <div class="layout">
        ...
        <div class="main-area">
            <header class="topbar"><button class="sidebar-toggle" onclick="document.getElementById('sidebar').classList.toggle('open')"><i class="bi bi-list"></i></button><span class="topbar-title">Tambah Tindakan</span></header>
            <main class="page-content">
                <div class="page-header">
                    <h1>Tambah Tindakan Baru</h1>
                    <p>Master data untuk dipakai pada rekam kesehatan.</p>
                    ...
                    <div class="form-card">
                        ...
                        <div class="form-actions"><button type="submit" class="btn-primary-custom"><i class="bi bi-check-lg"></i> Simpan Tindakan</button><button type="submit" name="action" value="save_new" class="btn-secondary-custom"><i class="bi bi-plus-circle"></i> Simpan &amp; Buat Baru</button><a href="index.php" class="btn-secondary-custom"><i class="bi bi-x-lg"></i> Batal</a></div>
                    </div>
                </form>
            </main>
        </div>
